"""
Golden Cross Rotation v4 — 3-State HMM Regime Adaptation
=========================================================
...
Regime-adaptive parameters via 3-state HMM (Bull/Sideways/Crisis):
  BULL:     Top 5, ATR trailing stop, 1.0× sizing, 10-day min hold
  SIDEWAYS: Top 4, 15% fixed stop, 0.70× sizing, 7-day min hold
  CRISIS:   Top 3, 10% fixed stop, 0.40× sizing, 5-day min hold
...
Usage:
  cd backend && ./venv/bin/python ../strategies/golden_cross_v4_hmm.py
"""
...
END = "2026-07-08"
CAPITAL = 100_000.0
MAX_HOLDINGS = 5  # Default, overridden by regime
ANGLE_WEIGHT = 0.60
CAP_WEIGHT = 0.40
MIN_HOLD_DAYS = 10  # Default, overridden by regime
SIZING_PCTS = [0.30, 0.25, 0.20, 0.15, 0.10]

# Dynamic trailing stop parameters (used in BULL regime)
ATR_PERIOD = 14
ATR_MULTIPLIER = 3.0
...
def main():
    ...
    from app.services.markov.hmm_regime_model import HMMRegimeManager
    from app.db.database import engine
    from sqlalchemy import text
    from app.utils.security import get_safe_table_name

    print("=" * 80)
    print("  GOLDEN CROSS ROTATION v4 — 3-STATE HMM REGIME ADAPTATION")
    print("=" * 80)
    print(f"  Period:     {AS_OF} → {END}")
    print(f"  Capital:    ${CAPITAL:,.2f}")
    print(f"  Holdings:   Regime-adaptive (Bull:5 / Sideways:4 / Crisis:3)")
    print(f"  Ranking:    {ANGLE_WEIGHT*100:.0f}% trend + {CAP_WEIGHT*100:.0f}% market cap")
    print(f"  Trailing:   Bull=ATR dynamic / Sideways=15% fixed / Crisis=10% fixed")
    print(f"  Min Hold:   Regime-adaptive (Bull:10d / Sideways:7d / Crisis:5d)")
    print(f"  Sizing:     Regime-adaptive (Bull:1.0× / Sideways:0.70× / Crisis:0.40×)")
    ...
    print(f"  Regime:     3-state HMM (SPY returns + VIX + MA200 slope)")
    print("=" * 80)
    ...
    # ── HMM regime training (walk-forward) ───────────────────────────────────
    print("\n🧠 Training 3-state HMM regime models (walk-forward)...")
    hmm_manager = HMMRegimeManager(n_states=3, n_iter=1000)
    hmm_manager.train_walk_forward(AS_OF, END, retrain_interval_days=180)

    # Pre-compute regime params for every trading day
    print("\n📊 Pre-computing regime parameters for all trading days...")
    all_dates = sorted(daily_crossovers.keys())
    regime_params_cache = {}
    for d in all_dates:
        regime_params_cache[d] = hmm_manager.get_regime_params(d)

    # Count regime distribution
    regime_counts = {}
    for d, p in regime_params_cache.items():
        r = p["regime"]
        regime_counts[r] = regime_counts.get(r, 0) + 1
    print(f"  Regime distribution across {len(all_dates)} trading days:")
    for r, c in sorted(regime_counts.items(), key=lambda x: -x[1]):
        print(f"    {r}: {c} days ({c/len(all_dates)*100:.1f}%)")
    ...
    for sim_idx, current_date in enumerate(all_dates):
        ...
        # Get regime-specific parameters for this date
        rp = regime_params_cache[current_date]
        regime = rp["regime"]
        max_holdings = rp["max_holdings"]
        min_hold_days = rp["min_hold_days"]
        sizing_mult = rp["sizing_mult"]
        use_atr_stop = rp["use_atr_stop"]
        fixed_stop_pct = rp["trailing_stop_pct"]
        ...
        for ticker in list(holdings.keys()):
            ...
            # ── Trailing stop (regime-adaptive) ──────────────────────────
            if use_atr_stop:
                # ATR-based dynamic stop (BULL regime)
                atr_value = get_atr(ticker, current_date)
                trailing_stop_pct = compute_dynamic_trailing_stop(atr_value, current_price)
            else:
                # Fixed stop (SIDEWAYS or CRISIS regime)
                trailing_stop_pct = fixed_stop_pct if fixed_stop_pct is not None else 0.15
            ...
            if dd >= trailing_stop_pct:
                ...
                trades.append({
                    ...
                    "return_pct": round(ret * 100, 2),
                    "pnl_dollars": round(pnl, 2),
                    "exit_reason": f"Stop ({dd:.1%}) [{regime}]",
                })
                cash += h["shares"] * current_price
                to_remove.append(ticker)
                continue
        ...
        top_n = all_stocks[:max_holdings]
        top_n_tickers = set(s["ticker"] for s in top_n)

        # ── 4. Sell dropped (with regime-adaptive min hold) ────────────────
        to_drop = [t for t in holdings if t not in top_n_tickers]
        ...
        # ── 5. Buy new top N (regime-adaptive sizing) ──────────────────────
        for rank, s in enumerate(top_n):
            if s["ticker"] not in holdings:
                ...
                target_pct *= sizing_mult  # Apply regime sizing multiplier
