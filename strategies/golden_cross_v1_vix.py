"""
Golden Cross Rotation — V1: VIX Volatility Overlay
====================================================
...
VIX-based sizing multiplier reduces position sizes when volatility is high.
...
VIX Multiplier:
  VIX < 15:  1.0x (full size)
  VIX 15-20: 0.8x
  VIX 20-30: 0.5x
  VIX > 30:  0.25x

Results (2020-01-01 → 2026-07-08):
  Return:  +615.58%  (vs SPY +151.72%)
  Alpha:   +463.86%
  Final:   $715,576

Usage:
  cd backend && ./venv/bin/python ../strategies/golden_cross_v1_vix.py
"""
...
# VIX thresholds for sizing multiplier
VIX_THRESHOLDS = [
    (15, 1.0),   # VIX < 15: full size
    (20, 0.8),   # VIX 15-20: 80%
    (30, 0.5),   # VIX 20-30: 50%
    (float('inf'), 0.25),  # VIX > 30: 25%
]
...
def get_vix_multiplier(vix_close: float) -> float:
    """Return position sizing multiplier based on VIX level."""
    for threshold, mult in VIX_THRESHOLDS:
        if vix_close < threshold:
            return mult
    return 0.25
...
def main():
    ...
    print("=" * 80)
    print("  GOLDEN CROSS ROTATION — V1: VIX VOLATILITY OVERLAY")
    print("=" * 80)
    ...
    print(f"  VIX Overlay: Active (thresholds: <15=1.0x, 15-20=0.8x, 20-30=0.5x, >30=0.25x)")
    print("=" * 80)
    ...
    with engine.connect() as conn:
        vix_rows = conn.execute(
            text(f'SELECT "Date", "Close" FROM vix WHERE "Date" >= \'2018-01-01\' AND "Date" <= \'{END}\' ORDER BY "Date"')
        ).fetchall()
    for row in vix_rows:
        vix_cache[str(row[0])] = float(row[1])
    print(f"  {len(vix_cache)} VIX data points loaded.")

    def get_vix_close(date_str: str) -> float:
        """Get VIX close price for a given date, with fallback to nearest prior date."""
        if date_str in vix_cache:
            return vix_cache[date_str]
        # Fallback: find the most recent VIX date before this one
        all_dates = sorted(vix_cache.keys())
    ...
    print(f"\n📈 Running daily rotation simulation with VIX overlay...")
    ...
    for sim_idx, current_date in enumerate(all_dates):
        ...
        vix_close = get_vix_close(current_date)
        vix_mult = get_vix_multiplier(vix_close)
        ...
        # ── 5. Buy new top 5 (dynamic sizing × VIX multiplier) ────────────
        for rank, s in enumerate(top5):
            if s["ticker"] not in holdings:
                ...
                target_value = portfolio_value * target_pct * vix_mult
    ...
    print(f"\n{'='*80}")
    print("  RESULTS (WITH VIX OVERLAY)")
    print("=" * 80)
