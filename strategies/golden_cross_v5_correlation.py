"""
Golden Cross Rotation v5 — Correlation-Based Position Capping
==============================================================
Scans 1500 stocks daily for:
  Entry A (golden cross): EMA20 crosses above EMA200
  Entry B (trend ride):   Price > EMA50 > EMA200 AND RSI(14) > 60 AND volume > 1.2x 50d avg
...
Dynamic trailing stop = clamp(ATR(14) x 3 / price, 8%, 30%) — recalculated daily.
Death cross exit.
Markov regime adaptation for bear market protection.
Correlation-based position capping: reduces correlated positions (>0.70 rho) by 30%.
$100,000 starting capital.

Usage:
  cd backend && ./venv/bin/python ../strategies/golden_cross_v5_correlation.py
"""
...
# Correlation capping parameters
CORR_LOOKBACK = 60          # rolling window for return correlation
CORR_THRESHOLD = 0.70       # correlation threshold to flag a pair
CORR_REDUCTION_PCT = 0.30   # reduce the smaller position by this fraction
...
def compute_dynamic_trailing_stop(atr_value, current_price):
    ...
    Formula: clamp(ATR(14) x multiplier / current_price, min, max)
...
def compute_correlation(ticker1, ticker2, stock_db, current_date, lookback=CORR_LOOKBACK):
    """
    Compute the rolling price-return correlation between two tickers.

    Uses the last `lookback` trading days of daily returns to compute
    the Pearson correlation coefficient. Returns 0.0 if insufficient data.
    """
    d1 = stock_db.get(ticker1)
    d2 = stock_db.get(ticker2)
    if d1 is None or d2 is None:
        return 0.0

    # Build Series with date index
    s1 = pd.Series(d1["close"], index=pd.to_datetime(d1["dates"]), name=ticker1)
    s2 = pd.Series(d2["close"], index=pd.to_datetime(d2["dates"]), name=ticker2)
    ...
    # Take the last `lookback + 5` rows (extra for pct_change)
    ...
    if len(returns) < 20:  # Need minimum data points for meaningful correlation
        return 0.0
...
def apply_correlation_capping(holdings, stock_db, current_date, cash):
    """
    Apply correlation-based position capping to the current holdings.

    For each pair of holdings with correlation > CORR_THRESHOLD:
      1. Reduce the position with the lower score by CORR_REDUCTION_PCT
      2. Redistribute the freed capital to the holding with the lowest
         average correlation to all other holdings
    ...
    # Local get_price that captures stock_db from the parameter
    ...
    for i in range(len(tickers)):
        for j in range(i + 1, len(tickers)):
            ...
            corr = compute_correlation(t1, t2, stock_db, current_date)
            all_corrs[(t1, t2)] = corr

    # -- 2. Find flagged pairs (correlation > threshold) ---------------------
    flagged = [(t1, t2, c) for (t1, t2), c in all_corrs.items() if c > CORR_THRESHOLD]

    if not flagged:
        return cash

    # -- 3. Compute average correlation for each holding (for redistribution) -
    avg_corrs = {}
    for t in tickers:
        vals = []
        for (a, b), c in all_corrs.items():
            if a == t:
                vals.append(c)
            elif b == t:
                vals.append(c)
        avg_corrs[t] = np.mean(vals) if vals else 0.0

    # -- 4. Process each flagged pair -----------------------------------------
    for t1, t2, corr in flagged:
        if t1 not in holdings or t2 not in holdings:
            continue

        h1 = holdings[t1]
        h2 = holdings[t2]

        # Reduce the position with the lower score by CORR_REDUCTION_PCT
        if h1["score"] <= h2["score"]:
            reduce_ticker = t1
        else:
            reduce_ticker = t2

        h = holdings[reduce_ticker]
        reduce_shares = int(h["shares"] * CORR_REDUCTION_PCT)
        if reduce_shares <= 0:
            continue

        current_price = _get_price(reduce_ticker, current_date)
        if current_price <= 0:
            continue

        freed_capital = reduce_shares * current_price
        h["shares"] -= reduce_shares
        cash += freed_capital

        # Find the least-correlated holding (excluding the reduced one)
        candidates = {t: c for t, c in avg_corrs.items() if t != reduce_ticker}
        if not candidates:
            continue

        best_ticker = min(candidates, key=candidates.get)

        # Add freed capital to the best ticker
        price = _get_price(best_ticker, current_date)
        if price > 0:
            additional_shares = int(freed_capital / price)
            if additional_shares > 0:
                cost = additional_shares * price
                if cost <= cash:
                    holdings[best_ticker]["shares"] += additional_shares
                    cash -= cost
...
def main():
    ...
    print("=" * 80)
    print("  GOLDEN CROSS ROTATION v5 — CORRELATION-BASED POSITION CAPPING")
    print("=" * 80)
    ...
    print(f"  Trailing:   Dynamic ATR(14)x{ATR_MULTIPLIER} / price  [{TRAILING_STOP_MIN:.0%}-{TRAILING_STOP_MAX:.0%}]")
    ...
    print(f"  Entry B:    Price>EMA50>EMA200 + RSI>60 + Vol>1.2xavg (EMA50 slope)")
    print(f"  Correlation: {CORR_LOOKBACK}d rolling, threshold >{CORR_THRESHOLD:.0%}, "
          f"reduce {CORR_REDUCTION_PCT:.0%}")
    print("=" * 80)
    ...
    for sim_idx, current_date in enumerate(all_dates):
        if (sim_idx + 1) % 200 == 0:
            print(f"  Day {sim_idx+1}/{len(all_dates)}  Portfolio: ${portfolio_value:>9,.2f}  "
                  f"Holdings: {len(holdings)}  CorrActions: {total_corr_actions}")

        regime = get_regime(current_date)

        # -- Crisis Override: go to cash when SPY drops >20% from 200-day high ---
        spy_cp = get_price("SPY", current_date)
        if spy_cp > 0:
            spy_prices.append(spy_cp)
            if len(spy_prices) > 200:
                spy_prices.pop(0)
            spy_high = max(spy_prices) if spy_prices else spy_cp
        ...
        # -- 1. Check exits (death cross, trailing stop) -------------------------
        to_remove = []
        for ticker in list(holdings.keys()):
            h = holdings[ticker]
        ...
        # -- 6. Correlation-based position capping -------------------------------
        if len(holdings) >= 2:
            old_cash = cash
            cash = apply_correlation_capping(holdings, stock_db, current_date, cash)
            if cash != old_cash:
                total_corr_actions += 1
    ...
    print(f"  Corr Actions:     {total_corr_actions}")
    ...
    if spy_df is not None and not spy_df.empty:
        ...
        print(f"  Alpha:            {alpha:>+8.2f}%  {'[OK]' if alpha > 0 else '[WARN]'}")
